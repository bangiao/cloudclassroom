...
public class IdCardUtils {
     ...
     * @return 错误信息或成功信息
     */
    public static String check(String idCardStr) {
        String idCard = idCardStr;
        ...
        if (idCardStr.length() == 18) {
            idCardStr = idCardStr.substring(0, 17);
        } else {
            idCardStr = idCardStr.substring(0, 6) + "19" + idCardStr.substring(6, 15);
        }

        if (!isNumeric(idCardStr)) {
            return LASTOFNUMBER;
        }

        // 判断出生年月是否有效
        String strYear = idCardStr.substring(6, 10);// 年份
        String strMonth = idCardStr.substring(10, 12);// 月份
        String strDay = idCardStr.substring(12, 14);// 日期
        ...
        if (areaCode.get(idCardStr.substring(0, 2)) == null) {
            return CODINGERROR;
        }

        if (!isValidCode(idCardStr, idCard)) {
     ...
     */
    private static boolean isValidCode(String idCardStr, String idCard) {
        String[] validCode = {"1", "0", "X", "9", "8", "7", "6", "5", "4", "3", "2"};
        String[] Wi = {"7", "9", "10", "5", "8", "4", "2", "1", "6", "3", "7", "9", "10", "5", "8", "4", "2"};
        int sum = 0;
        for (int i = 0; i < 17; i++) {
            sum = sum + Integer.parseInt(String.valueOf(idCardStr.charAt(i))) * Integer.parseInt(Wi[i]);
        }
        int modValue = sum % 11;
        String strVerifyCode = validCode[modValue];
        idCardStr = idCardStr + strVerifyCode;
        if (idCardStr.length() == 18) {
            return false;
        }
        return idCard.equals(idCardStr);
    }
